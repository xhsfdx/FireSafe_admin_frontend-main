            ...
            <el-form-item label="点位照片">
              ...
              >
                <div class="upload-area">
                  <i class="el-icon-camera upload-icon" />
...
export default {
  ...
  mounted() {
    ...
    this.loadProjectMaintenanceContent()
  },
  methods: {
    // 初始化项目信息
    initProjectInfo() {
    ...
    // 获取项目的维保内容并自动勾选
    async loadProjectMaintenanceContent() {
      try {
        if (!this.projectInfo.projectId) {
          console.warn('缺少项目ID，无法获取维保内容')
          return
        }
        ...
        if (response.success && response.data && response.data.plan) {
          ...
          if (planData.maintainStardItem && planData.maintainStardItem.length > 0) {
            // 等待树形数据加载完成后再设置勾选
            this.$nextTick(() => {
              this.setTreeCheckedByProjectItems(planData.maintainStardItem)
            })
          } else {
            console.warn('项目没有配置维保内容')
            this.checkedMaintList = []
          }
        } else {
          console.warn('获取项目维保内容失败:', response.message)
          this.checkedMaintList = []
        }
      } catch (error) {
        console.error('获取项目维保内容失败:', error)
        this.$message.error('获取项目维保内容失败: ' + (error.message || '未知错误'))
        this.checkedMaintList = []
      }
    },

    // 根据项目维保项目ID设置树形控件勾选状态
    setTreeCheckedByProjectItems(maintenanceItemIds) {
      if (!this.$refs.maintTree || !this.maintTree.length) {
        console.warn('树形控件未初始化或数据未加载')
        return
      }
      ...
      this.maintTree.forEach(system => {
        system.children?.forEach(device => {
          device.children?.forEach(item => {
            if (maintenanceItemIds.includes(item.id)) {
        ...
        children: (system.devices || []).map(device => ({
          ...
          children: (device.items || []).map(item => ({
            ...
            data: {
              ...
              standard: item.maintainContent // 使用maintainContent作为规范内容
            }
          }))
        }))
      }))
    ...
    handleTreeCheck(data, { checkedKeys }) {
      console.log('树形控件勾选事件触发:', { data, checkedKeys })
    ...
    updateCheckedMaintList() {
      ...
      this.checkedMaintList = checkedNodes
        ...
        .map(n => ({
          id: n.data.id, // 维保项目ID
          systemCategory: n.data.system, // 消防系统/设施
          deviceName: n.data.item, // 维保项目
          maintainSlim: n.label, // 检测内容
          frequency: n.data.period, // 维保周期
          maintainContent: n.data.standard // 规范
        }))
        ...
        const requestData = {
          ...
          checkpoints: [{
            ...
            maintenanceItems: data.checkedMaintList.map(item => {
              // 根据数据结构，maintenanceItems应该是维保项目ID数组
              return item.id
            })
          }]
        }
