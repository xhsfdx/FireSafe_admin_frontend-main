      ...
      <el-table-column label="操作" align="center" width="200">
        <template slot-scope="{ row }">
          <template v-if="row.planDefinedStatus === '未制定'">
            ...
            <el-link type="danger" style="margin-left: 8px" @click="delRow(row)"
              >删除</el-link
            >
          </template>
          <template v-else>
            <el-link type="primary" @click="showProject(row)">项目详情</el-link>
            <el-link type="info" style="margin: 0 8px" @click="showPlan(row)"
              >任务详情</el-link
            >
...
export default {
  ...
  mounted() {
    ...
    // 监听计划状态更新事件
    this.$bus &&
      this.$bus.$on("plan-status-updated", this.handlePlanStatusUpdate);
  ...
  beforeDestroy() {
    // 移除事件监听器
    this.$bus &&
      this.$bus.$off("plan-status-updated", this.handlePlanStatusUpdate);
    // 清除定时器
  ...
  beforeRouteEnter(to, from, next) {
    next((vm) => {
      ...
      if (
        (from.name === "PointMaintenancePlan" ||
          from.name === "SystemMaintenancePlan") &&
        to.query.planUpdated
      ) {
        ...
        setTimeout(() => {
          if (to.query.rowIndex !== undefined) {
            // 使用行索引直接更新
            vm.updatePlanStatusByIndex(
              parseInt(to.query.rowIndex),
              "已制定",
              "进行中"
            );
          } else {
  ...
  methods: {
    ...
    createPlan(row) {
      this.$confirm(
        `确定要为项目"${row.projectName}"制定维护计划吗？`,
        "制定计划",
        {
        ...
        if (statusUpdate) {
          ...
          if (Date.now() - updateData.timestamp < 5 * 60 * 1000) {
            ...
            this.updatePlanStatus(
              updateData.planId,
              updateData.planDefinedStatus,
              updateData.planStatus
            );
    ...
    batchCreatePlans() {
      ...
      // 检查选中的记录是否都是未制定状态
      const unformulatedPlans = this.multipleSelection.filter(
      ...
      if (unformulatedPlans.length !== this.multipleSelection.length) {
        this.$message.warning(
          `选中的 ${this.multipleSelection.length} 条记录中，只有 ${unformulatedPlans.length} 条是未制定状态，将只为未制定的记录制定计划`
        );
      }

      this.$confirm(
        `确定要为选中的 ${unformulatedPlans.length} 条未制定记录制定计划吗？`,
        "批量制定计划",
        {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
    ...
    showBatchCreateDialog(groupedPlans) {
      const groupCounts = Object.keys(groupedPlans)
        .map((method) => `${method}: ${groupedPlans[method].length}条`)
        .join("，");
    ...
    processBatchCreate(groupedPlans) {
      ...
      // 显示进度提示
      this.$message.info(
        `开始批量制定 ${allPlans.length} 条计划，请按提示操作`
      );
